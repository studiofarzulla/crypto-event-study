...
An earlier version of this work answered the question affirmatively at the second moment \citep{Farzulla2025Infra}. Estimating GJR-GARCH-X models on a six-asset panel, it reported that infrastructure events generate a $5.7\times$ larger conditional-variance response than regulatory events, with an apparently decisive significance level.\footnote{That predecessor circulates as a Research Square preprint (10.21203/rs.3.rs-8323026). To keep the public record consistent with the present re-analysis, on acceptance we will annotate it with a correction note pointing to this paper and its overturned significance, so the earlier $p<0.001$ claim is not left standing uncorrected; the first-moment companion preprint (Ref.~\citealp{Farzulla2025SameReturns}) is superseded in the same way.} That $5.7\times$ is the prior paper's high-salience-curated estimate ($\bar\delta_{\text{infra}}/\bar\delta_{\text{reg}}=2.385/0.419$) under its canonical estimator and rolling winsorisation; the present standalone pipeline does not reproduce it, and reports a curated point estimate of $4.88\times$ (global-clip winsorisation, canonical estimator), which is the headline figure throughout this paper (Section~\ref{sec:spec_stability}). The companion first-moment analysis \citep{Farzulla2025SameReturns} found no return-level difference, and the two were presented as a tidy story: the market differentiates shock types through the risk channel, not expected returns---structure that lives in the second moment.
...
\textbf{Overlapping windows.} Because events can fall within a few days of one another, their windows can overlap in calendar time, and we handle this explicitly rather than dropping the affected events. For the variance-side dummies over $[-3,+3]$ (Section~\ref{sec:methods_var}), a day falling in two windows of the \emph{same} type sets that type's indicator to one---the union, so no day is ever double-counted---while a day falling in both an infrastructure and a regulatory window leaves \emph{both} indicators active, so the GJR-GARCH-X estimates the two contributions jointly rather than assigning the shared day to either. For the first moment, each event's $[-5,+30]$ CAR is computed independently and then aggregated at the event level, so overlapping windows do not double-count; the dependence they induce between temporally close events is precisely what the event-level block bootstrap is built to preserve.
...
\textbf{The inference problem.} Pooling asset-event observations as i.i.d.\ inflates the degrees of freedom: with 50 events $\times$ 6 assets the naive count is nominally 300 (295 usable here, after BNB's later listing date drops five asset-event cells), but the true unit of independence is the event, and the assets within an event are strongly cross-correlated. A naive $t$-test on the pooled observations would therefore overstate power---the first-moment analogue of the pseudoreplication that inflated the variance headline.
...
For completeness, Table~\ref{tab:garch_params} reports the full per-asset variance-equation vector of the selected GJR-GARCH-X model---$\omega,\alpha,\gamma,\beta$, the Student-$t$ degrees of freedom $\nu$, the persistence $\alpha+\beta+\gamma/2$, and the two event coefficients $\delta_{\text{infra}},\delta_{\text{reg}}$ whose cross-asset means give the curated $4.88\times$ multiplier. (The sentiment-decomposition coefficients are reported with the Granger analysis, Section~\ref{sec:granger}.)

\begin{table}[htbp]
\centering
\caption{Per-asset baseline GJR-GARCH-X variance-equation estimates (curated 50-event sample, global-clip winsorisation). Persistence is $\alpha+\beta+\gamma/2<1$. The cross-asset means $\bar\delta_{\text{infra}}=1.978$ and $\bar\delta_{\text{reg}}=0.405$ give the headline $4.88\times$ multiplier. Reproducible via \texttt{code/c14\_garch\_diagnostics.py}.}\label{tab:garch_params}
\small
\begin{tabular}{@{}lrrrrrrrr@{}}
\toprule
\textbf{Asset} & \boldmath$\omega$ & \boldmath$\alpha$ & \boldmath$\gamma$ & \boldmath$\beta$ & \boldmath$\nu$ & \textbf{Persist.} & \boldmath$\delta_{\text{infra}}$ & \boldmath$\delta_{\text{reg}}$ \\
\midrule
BTC & 0.115 & 0.074 & $-0.000$ & 0.925 & 3.13 & 0.999 & 1.008 & 0.298 \\
ETH & 0.127 & 0.070 & $-0.008$ & 0.925 & 3.76 & 0.999 & 2.247 & 0.488 \\
XRP & 1.326 & 0.213 & $-0.027$ & 0.772 & 3.14 & 0.999 & 2.237 & 1.219 \\
BNB & 0.286 & 0.139 & $-0.011$ & 0.855 & 4.21 & 0.999 & 1.206 & 0.181 \\
LTC & 0.711 & 0.098 & $-0.034$ & 0.884 & 3.93 & 0.999 & 2.380 & 0.144 \\
ADA & 1.404 & 0.159 & $-0.025$ & 0.801 & 4.58 & 0.972 & 2.792 & 0.099 \\
\midrule
Mean & --- & --- & --- & --- & --- & --- & \textbf{1.978} & \textbf{0.405} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{A self-consistency check, not a proof of correctness under the true DGP.} The Student-$t$-copula bootstrap is calibrated from the \emph{same} DGP the panels are drawn from, so its size \emph{must} land near nominal up to Monte-Carlo error---if it did not, the implementation would contain a bug. It comes out $0.039$ / $0.095$ against targets $0.05$ / $0.10$, both within $\approx 1.7$ standard errors of nominal (mildly conservative at $5\%$, essentially exact at $10\%$). What this certifies is narrower than it may appear: the simulator is correctly implemented, and the procedure is correctly sized \emph{under a DGP matched to the data's salient features}---heavy Student-$t$ margins at $\nu\approx 3$ and the estimated cross-asset correlation. It does not, and cannot, establish correct size under the true, unknown DGP; we have moved the modelling assumptions out of the test and into the generator, and the test inherits them. We therefore do not leave robustness to that as an assumption: Table~\ref{tab:size_perturb} re-runs the size loop under perturbed data-generating processes. What the check does establish is still decisive for the ladder: the over-rejection of rungs (i)--(iii) is a real property of those procedures, not an artefact of a broken simulator.

\textbf{Robustness to DGP perturbation.} Table~\ref{tab:size_perturb} re-estimates empirical size under four alternatives to the fitted DGP: lighter tails ($\nu=8$), heavier tails ($\nu=2.5$), a near-Gaussian dependence structure (copula $\nu_c=200$), and a deliberately \emph{mis}-specified case in which panels are drawn with heavy $\nu=2.5$ tails but the bootstrap critical values are calibrated at the lighter fitted $\nu\approx 3.8$. Two things hold and one is a useful caveat. First, the naive i.i.d.\ rule over-rejects a true null at $0.48$--$0.59$ in \emph{every} scenario---its failure is a property of ignoring cross-asset dependence, not of any particular tail or copula choice, so the central diagnostic does not depend on a flattering DGP. Second, under a correctly specified generator the $t$-copula bootstrap controls size throughout ($0.03$--$0.06$ across the matched rows), while the Gaussian-copula bootstrap's failure tracks tail heaviness exactly as Lesson~3 predicts ($0.09$ at $\nu=8$ rising to $0.58$ at $\nu=2.5$). Third---the caveat---the $t$-copula is not magically immune to misspecification: when the true tails are heavier than its calibration it over-rejects ($0.29$, last row). This is the honest limit of the procedure, but it cuts in the conclusion's favour: a test that becomes \emph{anti}-conservative (too eager to reject) under heavier-than-fitted tails, yet still returns $p=0.322$ on the data, makes the failure to reject \emph{more} secure, not less---an over-eager test that nonetheless finds nothing is strong evidence for the null.

\begin{table}[htbp]
\centering
\caption{Empirical size (rejection rate of a true null at the $5\%$ level) under DGP perturbations. ``Matched'' rows calibrate and simulate under the same DGP; the last row mis-specifies (simulate $\nu=2.5$, calibrate at the fitted $\nu$). $N\approx 250$--$290$ usable panels per row, $B_{\text{ref}}=2000$; reproducible via \texttt{code/c10b\_dgp\_perturbation.py}.}\label{tab:size_perturb}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{DGP scenario} & \textbf{Naive i.i.d.} & \textbf{Design-effect} & \textbf{Gaussian-copula} & \textbf{$t$-copula} \\
\midrule
Fitted $\nu$ ($\approx$ Table~\ref{tab:size_study} baseline) & 0.51 & 0.42 & 0.32 & \textbf{0.04} \\
Lighter tails ($\nu=8$) & 0.59 & 0.45 & 0.09 & \textbf{0.03} \\
Heavier tails ($\nu=2.5$) & 0.48 & 0.34 & 0.58 & \textbf{0.06} \\
Gaussian dependence ($\nu_c=200$) & 0.54 & 0.39 & 0.29 & \textbf{0.04} \\
\midrule
Mis-specified (sim $\nu=2.5$, calib fitted $\nu$) & 0.48 & 0.34 & 0.58 & \textit{0.29} \\
\bottomrule
\multicolumn{5}{@{}p{0.95\linewidth}@{}}{\footnotesize Nominal size is $0.05$. The naive rule over-rejects ($\approx 10\times$ nominal) in every scenario. The $t$-copula controls size under correctly specified DGPs (matched rows) and is anti-conservative only when the true tails are heavier than its calibration (last row)---which makes the paper's non-significant result conservative under that misspecification.}
\end{tabular}
\end{table}
...
\textbf{Statistical power.} The first moment is underpowered (MDE $\approx 19$--$20$ pp against an observed $\approx 7$ pp); the dual null is a failure to reject, not a demonstration of equality. The second moment has a non-trivial point estimate that the dependence-robust bootstrap cannot distinguish from zero; with an effective sample size of only $\approx 3$ it is power-limited in the same way. We do not quote a numerical minimum detectable effect for it, because a minimum detectable \emph{ratio} is not well-defined under the copula bootstrap---the null distribution of the multiplier is asymmetric and heavy-tailed, with a near-zero regulatory denominator---so the symmetry of the two power caveats is qualitative on the variance side rather than quantified. Both caveats are stated symmetrically throughout.
...
Two axes carry different messages and are worth separating. The variance multiplier is \emph{robust to inference choices}---the point estimate barely moves as the procedure is corrected from the naive $t$-test to the copula bootstrap; only the $p$-value does---yet \emph{fragile to identification choices}, sitting at $\approx 5\times$ under curated identification and $1.3$--$1.6\times$ under a mechanical screen. It is robust to inference, fragile to identification, and significant under neither. The ratio itself should be read with that fragility in mind: because the regulatory denominator $\bar\delta_{\text{reg}}$ sits near zero for part of the panel, the multiplier is a volatile summary---the anticipation-window sweep drives it to $8.77\times$ purely by shrinking that denominator---and the underlying difference $\bar\delta_{\text{infra}}-\bar\delta_{\text{reg}}$ in squared-percentage points is the steadier object. None of this is a finding of nothing. The second-moment point estimate runs the same way at both moments and concentrates where a mechanical-disruption mechanism would predict; the per-asset coefficients also agree in sign, though we are careful not to count that as six confirmations---it is the same cross-asset dependence that elsewhere collapses the effective sample size to roughly three, so the uniform sign is closer to one corroborating vote than six. What the study lacks, on fifty events and an effective handful of independent assets, is the power to separate a genuine if modest infrastructure-over-regulatory response from sampling noise. On balance the evidence tilts toward a real asymmetry this sample cannot resolve rather than toward equality---which is why the honest verdict is \emph{unresolved}, not \emph{absent}.
