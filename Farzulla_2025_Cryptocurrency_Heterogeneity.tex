...
\title{\textbf{Code Failures, Market Panic: \\
Why Infrastructure Events Hit Crypto Harder Than Regulations} \\
\vspace{0.5em}
\large A TARCH-X Analysis of Differential Volatility Responses}
...
\begin{center}
\small
\textbf{Paper DOI:} \href{https://doi.org/10.5281/zenodo.17595207}{10.5281/zenodo.17595207} \\
\textbf{Code DOI:} \href{https://doi.org/10.5281/zenodo.17595251}{10.5281/zenodo.17595251} \\
\vspace{0.5em}
\textbf{Paper License:} \ccby\ \href{https://creativecommons.org/licenses/by/4.0/}{CC BY 4.0} \\
\textbf{Code License:} \href{https://opensource.org/licenses/MIT}{MIT License}
\end{center}
...
\textbf{Main Findings:} Infrastructure events generate significantly larger volatility impacts than regulatory events (2.385\% vs 0.419\%, $p=0.0008$, Cohen's $d=2.753$), representing a 5.7$\times$ multiplier robust across multiple statistical tests including independent $t$-test ($t=4.768$), Mann-Whitney U ($p=0.0043$), and inverse-variance weighted analysis ($Z=3.64$, $p=0.0003$). Bayesian analysis provides strong confirmatory evidence with Bayes Factors exceeding 10 for 4 of 6 cryptocurrencies (strong to very strong evidence), with all 6 showing positive evidence for infrastructure dominance. Machine learning reveals three distinct response clusters through PCA (2 components explain 100\% variance) and hierarchical clustering. Network spillover analysis identifies ETH as the most central asset (eigenvector centrality 0.89) rather than BTC (0.71), with network density of 0.667 indicating substantial interconnectedness. Markov regime-switching models detect 5$\times$ sensitivity amplification during crisis periods ($F=45.23$, $p<0.001$). Parameter estimation enforces covariance stationarity via inequality constraints ($\alpha + \beta + |\gamma|/2 < 0.999$), with all six cryptocurrencies converging to the stationarity boundary ($\alpha+\beta \approx 0.999$), suggesting near-integrated variance dynamics characteristic of cryptocurrency markets. Substantial cross-sectional heterogeneity exists within infrastructure sensitivity, ranging from ADA (3.371\%) to BTC (1.191\%), a 2.18 percentage point spread. Raw significance (p < 0.05) holds for 5 of 6 cryptocurrencies for infrastructure events versus 0 of 6 for regulatory events, with only ETH infrastructure effect surviving FDR correction (q < 0.10).
...
\fbox{\begin{minipage}{0.9\textwidth}
\small
\textbf{Interactive Visualizations:} An interactive web-based dashboard allowing real-time exploration of all results is available at \href{https://www.farzulla.org/research/crypto-event-study/}{\texttt{https://www.farzulla.org/research/crypto-event-study/}}. The dashboard includes multiple visualization modes optimized for different devices and use cases.
\end{minipage}}
\end{center}
...
\subsubsection{Parameter Constraints and Robustness}

Parameter estimation enforces covariance stationarity via inequality constraints ($\alpha + \beta + |\gamma|/2 < 0.999$) with compatible parameter bounds ($\beta < 0.95$) during maximum likelihood optimization. Event coefficient parameters remain unbounded, allowing data-driven effect magnitude estimation.

This specification emerged from systematic robustness testing across three configurations: (i) bounded event coefficients ($\pm$1.0) artificially suppressed infrastructure impacts, yielding null results ($p=0.997$), (ii) unconstrained estimation without stationarity enforcement produced explosive variance processes ($\alpha+\beta > 1.0$) for 4/6 cryptocurrencies incompatible with classical GARCH theory, and (iii) the final specification balances theoretical validity (stationary variance) with empirical flexibility (unconstrained event effects).

The infrastructure-regulatory differential proves robust across all specifications (ratio 3.5--5.7$\times$), confirming our main finding survives methodological choices. All six cryptocurrencies converge to the stationarity constraint boundary ($\alpha+\beta \approx 0.999$), suggesting cryptocurrency variance dynamics exhibit near-integrated behavior characteristic of the asset class rather than modeling artifacts.
...
\subsection{Supplementary Statistical Methods}

Beyond the primary TARCH-X framework, we employ multiple advanced statistical approaches to validate and extend our findings:

\subsubsection{Bayesian Inference}

We implement Bayesian estimation using diffuse priors ($\mu \sim \mathcal{N}(0, 100)$, $\sigma^2 \sim \text{InverseGamma}(0.01, 0.01)$) to provide alternative inference robust to distributional assumptions. Markov Chain Monte Carlo (MCMC) sampling with 10,000 iterations after 2,000 burn-in generates posterior distributions for event coefficients. Bayes Factors quantify evidence strength: $BF_{10} > 10$ indicates strong evidence, $BF_{10} > 30$ very strong, and $BF_{10} > 100$ decisive evidence for differential impacts.

\subsubsection{Machine Learning Pattern Detection}

Principal Component Analysis (PCA) decomposes the 6-dimensional volatility response space to identify dominant patterns across cryptocurrencies. Hierarchical clustering using Ward's linkage on standardized event coefficients reveals natural groupings in response profiles. The optimal cluster count is determined via silhouette analysis and dendrogram inspection, providing unsupervised validation of event type distinctions.

\subsubsection{Network Spillover Analysis}

We construct correlation-based networks from conditional volatility series to examine systemic risk transmission. Edge weights represent pairwise correlations exceeding 0.3 threshold. Network metrics include: eigenvector centrality (influence based on connected node importance), betweenness centrality (information flow intermediation), clustering coefficient (local interconnectedness), and network density (overall connectivity). Time-varying analysis examines network evolution across market regimes.

\subsubsection{Markov Regime-Switching Models}

Two-state Markov-switching models capture regime-dependent volatility dynamics:
\begin{equation}
\sigma^2_t = \omega_{S_t} + \alpha_{S_t} \varepsilon^2_{t-1} + \beta_{S_t} \sigma^2_{t-1}
\end{equation}
where $S_t \in \{1,2\}$ denotes the latent regime. Maximum likelihood estimation via the EM algorithm identifies calm and crisis regimes. We test whether event impacts differ across regimes using interaction terms, with $F$-tests for regime-dependent sensitivity.
...
\begin{figure*}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figure1_enhanced_comparison.pdf}
...
\begin{figure*}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figure2_heterogeneity_enhanced.pdf}
...
\begin{figure*}[htbp]
\centering
\includegraphics[width=1.0\textwidth]{figure3_model_comparison.pdf}
\caption{Model Selection via Information Criteria}
\label{fig:model_comparison}

\footnotesize
AIC and BIC comparison across GARCH(1,1), TARCH(1,1), and TARCH-X specifications for all six cryptocurrencies. Winning models are marked with a gold star on black circular background for visibility. TARCH-X achieves lowest AIC for 5/6 cryptocurrencies (83\%), demonstrating that event-augmented specifications substantially improve model fit. The parsimony penalty in BIC still favors TARCH-X for 5/6 assets, confirming that added complexity is justified by improved explanatory power.
\end{figure*}
...
This challenges the conventional view of BTC as the primary systemic risk factor. ETH's smart contract ecosystem and DeFi integration make it the true network hub for volatility transmission.

\begin{figure*}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figure4_temporal_decomposition.pdf}
...
\item Regulatory effects remain stable (0.51\% vs 0.42\%)
...
The regime-switching analysis uncovers a stark asymmetry: infrastructure events amplify 5$\times$ during crisis periods (11.2\% vs 2.3\% volatility impact) while regulatory effects remain stable (0.51\% vs 0.42\%). This differential amplification reveals that mechanical disruptions and stressed market conditions interact synergistically, creating compound risks absent in calm periods.
...
Complete replication materials, including cleaned data, analysis code, and figure generation scripts, are archived on Zenodo with DOI: 10.5281/zenodo.17595207. The repository includes:
...
\item \textbf{Microstructure mechanisms:} Incorporating order flow imbalance, liquidity measures, and funding rates might better capture the mechanical channels through which infrastructure events propagate versus gradual information absorption during regulatory announcements.
...
All data, analysis code, figure generation scripts, and comprehensive documentation are publicly available at: \url{https://github.com/studiofarzulla/crypto-event-study}. The repository includes complete implementation details, parameter configurations, and instructions for reproducing all results. Data sources include CoinMarketCap historical price data and GDELT sentiment analysis. Models are implemented in Python 3.9+ and can be executed on standard hardware. The complete research package is archived on Zenodo with DOI: 10.5281/zenodo.17595207. An interactive dashboard for exploring all results is available at: \url{https://farzulla.org/research/crypto-event-study/dashboard.html}.
...
\textbf{Event Summary:} 50 events total (27 infrastructure, 23 regulatory) spanning January 2019 to August 2025, curated from an initial set of 207 events identified during research. The complete original event list is available in the GitHub repository. Events are listed chronologically below.
...
\subsection*{Major Changes from September 2025 Master's Thesis}

This November 2025 revision incorporates two critical corrections: removal of artificial constraints (October 2025) and stationarity bug fixes (November 10-12, 2025), revealing substantially different empirical findings compared to the original September 2025 Master's thesis submission.

\subsubsection*{Key Numerical Changes}

{\small\textbf{Hypothesis 1 (Infrastructure vs Regulatory):}}
\begin{itemize}[leftmargin=*,nosep]
\item SEPT 2025: Infrastructure 41.7\% vs Regulatory 41.5\%, $p=0.997$ (null result)
\item NOV 2025: Infrastructure 2.32\% vs Regulatory 0.42\%, $p=0.0057$ (highly significant)
\item Effect size: Cohen's $d = 2.88$ (huge effect)
\item 5.7$\times$ multiplier (infrastructure/regulatory)
\end{itemize}

\textbf{Cross-Sectional Rankings:}
\begin{itemize}[leftmargin=*,nosep]
\item SEPT 2025: BNB \#1 (0.947\%), LTC \#6 ($-0.027$\%), 97.4pp spread
\item NOV 2025: ADA \#1 (3.37\%), BTC \#6 (1.13\%), 2.24pp spread (infrastructure only)
\end{itemize}

\textbf{FDR Correction:}
\begin{itemize}[leftmargin=*,nosep]
\item NOV 2025: ETH infrastructure effect survives ($p=0.016$)
...
\textbf{NEW Conclusion:} ``Infrastructure events cause 5.7$\times$ larger volatility impacts than regulatory events ($p=0.0057$), robust across multiple statistical tests. Event type categorization provides substantial predictive power for volatility management.''
...
\item \textbf{Section 4.3} (H1 Results): Complete replacement with 5.7$\times$ multiplier finding, multiple statistical tests
...
\item \textbf{Section 6} (Final Remarks): Rewritten to emphasize practical applications of 5.7$\times$ multiplier
\end{itemize}
...
\item Infrastructure failures generate 5.7$\times$ larger market disruptions
...
\textbf{Zenodo DOI:} 10.5281/zenodo.17595207
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{robustness_effects_confidence_intervals.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{placebo_test_robustness.png}
