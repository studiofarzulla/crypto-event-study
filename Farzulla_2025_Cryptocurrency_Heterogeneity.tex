...
% Graphics path
\graphicspath{{outputs/publication/figures/}{publication_figures/}}
...
\textbf{Main Findings:} Infrastructure events generate significantly larger volatility impacts than regulatory events (2.385\% vs 0.419\%, $p=0.0053$, Cohen's $d=2.88$), representing a 5.7$\times$ multiplier robust across multiple statistical tests including independent $t$-test, Mann-Whitney U ($p=0.0043$), and inverse-variance weighted analysis ($Z=3.64$, $p=0.0003$). Parameter estimation enforces covariance stationarity via inequality constraints ($\alpha + \beta + |\gamma|/2 < 0.999$), with all six cryptocurrencies converging to the stationarity boundary ($\alpha+\beta \approx 0.999$), suggesting near-integrated variance dynamics characteristic of cryptocurrency markets. This finding supports the hypothesis that mechanical disruptions to trading infrastructure create more severe volatility responses than informational regulatory shocks. Substantial cross-sectional heterogeneity exists within infrastructure sensitivity, ranging from ADA (3.37\%) to BTC (1.19\%), a 2.18 percentage point spread. Only ETH infrastructure effects survive stringent Benjamini-Hochberg FDR correction ($p=0.016$), reflecting conservative multiple testing adjustment across 12 hypothesis tests rather than absence of genuine effects.
...
\midrule
Independent $t$-test & $t = 4.62$ & $0.0053^{**}$ & Highly significant \\
...
\footnotesize
Box plots comparing volatility impact coefficients for infrastructure events (red, $n=6$) versus regulatory events (green, $n=6$) across all cryptocurrencies. Infrastructure events show significantly larger mean impacts (2.385\% vs 0.419\%, $p=0.0053$, Cohen's $d=2.88$). Diamond markers indicate means, horizontal lines indicate medians. Individual data points (white circles) represent cryptocurrency-specific estimates with jitter for visibility. Statistical significance determined by two-sample $t$-test with unequal variances.
\end{figure*}
...
\footnotesize
Horizontal bar chart showing infrastructure event sensitivity coefficients across six major cryptocurrencies, sorted by magnitude. Bars in red indicate FDR-corrected significance at $q < 0.05$ (only ETH, $p=0.030$); gray bars indicate non-significant effects. Error bars represent standard errors. The 2.18 percentage point spread between ADA (3.37\%) and BTC (1.19\%) demonstrates substantial cross-sectional heterogeneity in cryptocurrency responses to infrastructure events.
\end{figure*}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.85\textwidth]{publication_figures/robustness_effects_confidence_intervals.png}
\caption{Infrastructure vs Regulatory Effects with Confidence Intervals across all six cryptocurrencies. Error bars represent 95\% confidence intervals.}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.85\textwidth]{publication_figures/robustness_cohens_d.png}
\caption{Effect Size (Cohen's d) demonstrating large effect sizes for the infrastructure-regulatory differential across all assets.}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.85\textwidth]{publication_figures/robustness_heterogeneity_ratio.png}
\caption{Heterogeneity Ratio Analysis showing consistent infrastructure dominance across cross-sectional comparisons.}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.85\textwidth]{publication_figures/robustness_rankings_heatmap.png}
\caption{Temporal Stability of Rankings. Cryptocurrency rankings remain stable across early (2019-2021) and late (2022-2025) periods, demonstrating structural rather than cyclical heterogeneity.}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.85\textwidth]{publication_figures/temporal_stability_analysis.png}
\caption{Temporal Stability Analysis. Perfect rank correlation (Spearman $\rho = 1.0$) between early and late period rankings confirms that cross-sectional heterogeneity reflects token-level structural characteristics rather than regime-dependent factors.}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.85\textwidth]{publication_figures/placebo_test_robustness.png}
\caption{Placebo Test Results. Distribution of placebo heterogeneity statistics from 1,000 randomly permuted samples. Observed heterogeneity (red line) significantly exceeds null distribution, confirming genuine event-driven effects.}
\end{figure}
