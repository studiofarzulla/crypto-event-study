...
\clearpage
\section*{Appendix E: Robustness Checks and Future Research}
\addcontentsline{toc}{section}{Appendix E: Robustness Checks and Future Research}
...
Future research with access to high-frequency sentiment data could investigate whether regulatory event impacts represent genuine market structure changes or merely sentiment-driven responses to regulatory theatre. The 5.7$\times$ differential suggests market participants distinguish between operational reality and regulatory signaling.

Additional research avenues include:

\begin{itemize}
\item \textbf{High-frequency analysis:} Intraday price and sentiment data could reveal the temporal dynamics of infrastructure vs regulatory shocks, distinguishing immediate liquidity impacts from slower information diffusion.
\item \textbf{Cross-market contagion:} Investigation of spillover effects between centralized exchanges, DeFi protocols, and traditional financial markets during infrastructure failures.
\item \textbf{Regulatory heterogeneity:} Comparative analysis across jurisdictions (US SEC vs EU MiCA vs Asia-Pacific regimes) to test whether regulatory clarity moderates volatility responses.
\item \textbf{Machine learning approaches:} Deep learning models trained on event embeddings could potentially improve event type classification beyond manual categorization.
\end{itemize}
