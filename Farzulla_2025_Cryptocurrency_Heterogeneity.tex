...
\title{\Large\bfseries Differential Volatility Responses to Infrastructure and Regulatory Events in Cryptocurrency Markets: A TARCH-X Analysis}

\author{
    \textbf{Murad Farzulla}\thanks{Corresponding author. Email: \href{mailto:contact@farzulla.org}{contact@farzulla.org}. ORCID: \href{https://orcid.org/0000-0002-3669-4621}{0000-0002-3669-4621}.} \\
    \small Farzulla Research \\
    \small \url{https://farzulla.org}
}
...
\textbf{Primary Hypothesis (H1): Asymmetric Volatility Response} -- Market structure/infrastructure events generate significantly larger volatility impacts than regulatory events due to immediate liquidity disruption versus gradual information absorption mechanisms.

\textbf{Secondary Hypothesis (H2): Sentiment Leading Indicator} -- News sentiment (GDELT-derived) serves as a leading indicator for volatility asymmetries, with infrastructure events showing immediate sentiment-volatility correlation versus regulatory events showing lagged responses.

\textbf{Methodological Hypothesis (H3): TARCH-X Superiority} -- TARCH-X models incorporating sentiment proxies outperform standard GARCH specifications in capturing asymmetric volatility responses to different event types in cryptocurrency markets.
...
\small
\begin{tabular}{@{}p{2cm}p{1cm}p{1.5cm}p{0.8cm}p{0.8cm}p{1.2cm}@{}}
\toprule
\textbf{Paper} & \textbf{Assets} & \textbf{Event Types} & \textbf{Window} & \textbf{Sentiment} & \textbf{Volatility Model} \\
\midrule
...
Caferra \& Vidal-Tomás (2021) & Multi & Infrastructure & $\pm$5d & Yes (GDELT) & GARCH \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Sentiment Indices and Leading Indicators in Digital Asset Markets}
...
\textbf{(A) SEC v Binance (June 5) and SEC v Coinbase (June 6, 2023):} Treated as a single regulatory episode with composite dummy $D_{\text{SEC\_enforcement}}$ for [June 2 to 9], recognising coordinated enforcement action.

\textbf{(B) Ethereum EIP-1559 (Aug 5) and Poly Network hack (Aug 10, 2021):} Retained as separate events with proportional weighting (0.5) during overlap period [Aug 7 to 8] to prevent double-counting whilst preserving distinct shock identification, acknowledging this approach treats overlapping days as equally attributable to both events.

\textbf{(C) Bybit hack (Feb 21) and SEC-Coinbase dismissal (Feb 27, 2025):} Cross-channel events handled through truncated windows, with Bybit [Feb 18 to 23], SEC dismissal [Feb 27 to Mar 2], and intervening days excluded.
...
\textbf{Model 3: TARCH-X with Event Dummies and Sentiment}
\begin{multline}
\sigma^2_t = \omega + \alpha_1 \varepsilon^2_{t-1} + \gamma_1 \varepsilon^2_{t-1} I(\varepsilon_{t-1}<0) + \beta_1 \sigma^2_{t-1} \\
...
Event volatility impacts are calculated as: Impact = $(\sigma^2_{\text{event}}/\sigma^2_{\text{baseline}}) - 1$
...
\textbf{Statistical Validation (Multiple Tests):}
...
\bottomrule
\multicolumn{6}{l}{\footnotesize \textbf{Spread:} 2.24 percentage points (ADA to BTC)}
\end{tabular}
\end{table}

\begin{figure*}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figure2_infrastructure_sensitivity.pdf}
...
\begin{figure*}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figure4_tarchx_performance.pdf}
...
\begin{equation*}
\sigma^2_t = \omega + \alpha \cdot \varepsilon^2_{t-1} + \gamma \cdot \varepsilon^2_{t-1} \cdot I(\varepsilon_{t-1} < 0) + \beta \cdot \sigma^2_{t-1} + \sum_j \delta_j \cdot x_{j,t}
\end{equation*}
...
\textbf{Hypothesis 1 (Infrastructure vs Regulatory):}
\begin{itemize}[leftmargin=*,nosep]
