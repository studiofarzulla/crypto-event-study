...
class PublicationOutputs:
    ...
    def _safe_get_model_stats(self, models: Dict, model_name: str) -> Tuple[float, float]:
        ...
        Args:
            ...
            model_name: Name of the model to extract stats from

        Returns:
            Tuple of (AIC, BIC), returns (np.nan, np.nan) if model doesn't exist or didn't converge
        """
    ...
    def generate_latex_model_comparison_table(self) -> str:
        ...
        for crypto, models in self.model_results.items():
            ...
            # Get model statistics using safe helper
...
"""
    ...
    def plot_event_impact_comparison(self):
        ...
        means = [ivw['Infrastructure']['weighted_average'],
                ivw['Regulatory']['weighted_average']]
        ci_lower = [ivw['Infrastructure']['ci_lower'],
                   ivw['Regulatory']['ci_lower']]
        ci_upper = [ivw['Infrastructure']['ci_upper'],
                   ivw['Regulatory']['ci_upper']]
        ...
        bars = ax.bar(x, means, width, yerr=errors, capsize=10,
        ...
        if 'difference' in ivw and ivw['difference']['significant_10pct']:
            ...
            sig_text = '***' if ivw['difference']['p_value'] < 0.01 else \
                      '**' if ivw['difference']['p_value'] < 0.05 else '*'
    ...
    def export_all_to_csv(self):
        ...
        if 'fdr_correction' in self.analysis_results:
            if isinstance(self.analysis_results['fdr_correction'], pd.DataFrame):
                self.analysis_results['fdr_correction'].to_csv(
                    csv_dir / 'event_impacts_fdr.csv', index=False)
...
def generate_publication_outputs(model_results: Dict, analysis_results: Dict,
